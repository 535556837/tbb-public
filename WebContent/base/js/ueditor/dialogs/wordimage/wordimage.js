		...
		for (var i = 0,img; img = images[i++];) {
			...
			for (var j = 0,url; url = imageUrls[j++];) {
				...
				if (src.indexOf(url.title.replace(" ","")) != -1) {
				 ...
				 var inde= imageActionUrl.indexOf("base");
	                imageActionUrl=imageActionUrl.substring(0,inde);
	               var link = imageActionUrl+"commonAction/getimg?fileName=" + url.detail;
				
					img.src = urlPrefix + link;
					img.setAttribute("_src", urlPrefix + link);  //同时修改"_src"属性
